...
def YUVread_16bit(path, size, frame_num=None, start_frame=0, mode='420'):
    """
    read 10bit，12bit or 16bit YUV
    Only for 4:2:0 and 4:4:4 for now.
    ...
    with open(path, 'rb') as file:
        ...
        if frame_num is None:
            ...
            while True:
                if mode == '420':
                    y_tmp = np.uint16((list(file.read(height * width * 2))))
                    y_low = y_tmp[::2]
                    y_high = y_tmp[1::2]
                    y = (y_high << 8) + y_low
                    u_tmp = np.uint16((list(file.read((height * width >> 2) *2))))
                    u_low = u_tmp[::2]
                    u_high = u_tmp[1::2]
                    u = (u_high << 8) + u_low
                    v_tmp = np.uint16(list(file.read((height * width >> 2) * 2)))
                    v_low = v_tmp[::2]
                    v_high = v_tmp[1::2]
                    v = (v_high << 8) + v_low
                else:
                    ...
                    y_high = y_tmp[1::2]
                    y = (y_high << 8) + y_low
                    u_tmp = np.uint16((list(file.read((height * width) *2))))
                    u_low = u_tmp[::2]
                    u_high = u_tmp[1::2]
                    u = (u_high << 8) + u_low
                    v_tmp = np.uint16(list(file.read((height * width) * 2)))
                    v_low = v_tmp[::2]
                    v_high = v_tmp[1::2]
                    v = (v_high << 8) + v_low
                if y.shape == (0,):
